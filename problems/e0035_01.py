...
SOLVED = True
SPEED = 0.30
TAGS = ['prime_sieve', 'primes', 'circular_prime', 'digit_rotation']
...
# It is significantly faster to test if a candidate is in a set than a list.
# Performance could be improved by not retesting the rotations in the outer loop.
def main():
    """Solve problem."""
    ...
    total = 0
    known = set(Primes.known_primes)
    for prime in Primes.known_primes:
        # print 'Prime:', prime
        digits = int(math.log10(prime))
        rotation = prime
        for dummy in xrange(digits):
            last = rotation % 10
            rotation = last * 10 ** digits + rotation/10
            # print ' - %d' % (rotation)
            if rotation not in known:
                break
        else:
            print prime
            total += 1
